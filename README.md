...
Causa: ao haver a deleção do registro o cursor do dataset muda, fazendo com que um registro fosse "pulado".
Solução: se o registro não for par(não for deletado), deve ir para o próximo registro.
...
Causa: há uma limitação de memória para aplicações Win32, onde o bloco máximo de memória a ser alocada para um endereço é de 2 giga; sendo assim, o DataSet "estoura" isto a medida que os PDFs são carregados. Através da diretiva IMAGE_FILE_LARGE_ADDRESS_AWARE é possível incrementar isto para 3 giga, porém não resolve o problema.
Solução: habilitada diretiva IMAGE_FILE_LARGE_ADDRESS_AWARE conforme descreve o documento "http://docwiki.embarcadero.com/RADStudio/Sydney/en/Increasing_the_Memory_Address_Space" e implementado controle para quando o tamanho dos dados contidos no DataSet for maior que 500mb fazer o envio para o servidor(a primeira ideia era ter feito o envio de arquivo por arquivo, porém não achei muito performático).
...
Causa: a deleção dos arquivos só ocorria quando houvesse dados no DataSet repassados ao servidor, neste caso não havia, não fazendo a deleção do arquivo(o que também era feito arquivo a arquivo).
Solução: ao enviar os dados para o servidor a pasta é de arquivos do servidor limpa, sendo que em caso de exceção a mesma pasta será limpa; em caso de não ocorrer exceção os arquivos serão mantidos(entendo que o servidor é stateless, sendo assim não poderia criar métodos, por exemplo "IniciarEnvioBloco" e "FinalizarEnvioBloco", para eliminar os arquivos em caso de erro).